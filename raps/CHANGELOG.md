...
- **Monorepo Architecture**: Consolidated all workspace crates into single `raps/` repository
  - All service crates (`raps-kernel`, `raps-oss`, `raps-derivative`, `raps-dm`, `raps-ssa`) now in monorepo
  - Community and Pro tier crates consolidated
  - CLI binary crate (`raps`) integrated into monorepo
- **Workspace Organization**: Unified Cargo workspace with shared dependencies and build profiles
- **Git Consolidation**: Removed separate git repositories, unified history in monorepo
- **Archive Structure**: Moved obsolete repositories to `archive/` directory

#### Distribution Channel Updates
- **Homebrew Tap**: Updated to version 3.3.0
- **Scoop Bucket**: Updated to version 3.3.0
- **Docker Image**: Updated to version 3.3.0
- **GitHub Action**: Uses dynamic version detection (no update needed)

### Changed
- **Repository Structure**: All workspace crates moved into `raps/` folder
- **Build Configuration**: Unified `.cargo/config.toml` with fast linker settings
- **Dependency Management**: Workspace-level dependency sharing for consistency

### Internal
- Integration tests for atomic cross-module changes
- Workspace-level linting and formatting rules
