...
- Rust 1.88, Edition 2024 + reqwest (HTTP), tokio (async runtime), clap (CLI), serde (serialization) — all existing workspace deps (001-fix-api-alignment-bugs)
- N/A (no storage changes; token caching uses existing in-memory + keyring) (001-fix-api-alignment-bugs)
...
## Recent Changes
- 001-fix-api-alignment-bugs: Added Rust 1.88, Edition 2024 + reqwest (HTTP), tokio (async runtime), clap (CLI), serde (serialization) — all existing workspace deps
