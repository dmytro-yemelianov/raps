...
impl RealityCommands {
    pub async fn execute(self, client: &RealityCaptureClient) -> Result<()> {
        match self {
            RealityCommands::Create { name, scene_type, format } => {
                create_photoscene(client, name, scene_type, format).await
            }
            RealityCommands::Upload { photoscene_id, photos } => {
            ...
            RealityCommands::Status { photoscene_id, wait } => {
                check_status(client, &photoscene_id, wait).await
            }
            RealityCommands::Result { photoscene_id, format } => {
...
async fn create_photoscene(
    ...
    let selected_scene_type = match scene_type {
        ...
        None => {
            ...
            let selection = Select::new()
                ...
                .items(&types)
                .interact()?;
            if selection == 0 { SceneType::Aerial } else { SceneType::Object }
        }
    };
    ...
    let photoscene = client.create_photoscene(&scene_name, selected_scene_type, selected_format).await?;
    ...
    println!("\n{}", "Next steps:".yellow());
    println!("  1. Upload photos: raps reality upload {} <photo1.jpg> <photo2.jpg> ...", photoscene.photoscene_id);
    println!("  2. Start processing: raps reality process {}", photoscene.photoscene_id);
    println!("  3. Check status: raps reality status {} --wait", photoscene.photoscene_id);

    Ok(())
}

async fn upload_photos(client: &RealityCaptureClient, photoscene_id: &str, photos: Vec<PathBuf>) -> Result<()> {
...
async fn start_processing(client: &RealityCaptureClient, photoscene_id: &str) -> Result<()> {
    ...
    println!("  {}", "Use 'raps reality status <id> --wait' to monitor progress".dimmed());
    Ok(())
}

async fn check_status(client: &RealityCaptureClient, photoscene_id: &str, wait: bool) -> Result<()> {
    if wait {
...
async fn get_result(client: &RealityCaptureClient, photoscene_id: &str, format: &str) -> Result<()> {
    ...
    if let Some(ref link) = result.scene_link {
        ...
        println!("{}", "No download link available yet. Processing may still be in progress.".yellow());
    }
...
fn list_formats(client: &RealityCaptureClient) -> Result<()> {
    ...
    for format in formats {
        println!("  {} {} - {}", "•".cyan(), format, format.description().dimmed());
    }
...
async fn delete_photoscene(client: &RealityCaptureClient, photoscene_id: &str) -> Result<()> {
    ...
    println!("{} Photoscene '{}' deleted!", "✓".green().bold(), photoscene_id);
    Ok(())
}
