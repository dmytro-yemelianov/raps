...
mod integration_tests {
    ...
    /// Create a Derivative client configured to use the mock server
    fn create_mock_derivative_client(mock_url: &str) -> DerivativeClient {
        let config = Config {
    ...
    async fn test_translate_svf2_success() {
        ...
        Mock::given(method("POST"))
            ...
            .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({
                "result": "success",
                "urn": "dXJuOmFkc2sub2JqZWN0czpvcy5vYmplY3Q6dGVzdC1idWNrZXQvbW9kZWwucnZ0",
                "acceptedJobs": {
                    "output": {
                        "formats": [
                            {"type": "svf2"}
        ...
        let client = create_mock_derivative_client(&server.uri());
        let result = client
            .translate(
                "dXJuOmFkc2sub2JqZWN0czpvcy5vYmplY3Q6dGVzdC1idWNrZXQvbW9kZWwucnZ0",
                OutputFormat::Svf2,
                None,
            )
            .await;
        ...
        assert!(response.accepted_jobs.is_some());
    }
    ...
    async fn test_translate_with_root_filename() {
        ...
        Mock::given(method("POST"))
            ...
            .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({
                "result": "success",
                "urn": "dXJuOmFkc2sub2JqZWN0czpvcy5vYmplY3Q6dGVzdC1idWNrZXQvYXJjaGl2ZS56aXA"
            })))
            .mount(&server)
            .await;
        ...
        let result = client
            .translate(
                "dXJuOmFkc2sub2JqZWN0czpvcy5vYmplY3Q6dGVzdC1idWNrZXQvYXJjaGl2ZS56aXA",
                OutputFormat::Svf2,
                Some("model.rvt"),
            )
            .await;
    ...
    #[tokio::test]
    async fn test_translate_obj_format() {
        let server = MockServer::start().await;
        setup_auth_mock(&server).await;
        ...
        let client = create_mock_derivative_client(&server.uri());
        let result = client.translate("test-urn", OutputFormat::Obj, None).await;

        assert!(result.is_ok());
    }

    #[tokio::test]
    async fn test_translate_unauthorized() {
        let server = MockServer::start().await;
        setup_auth_mock(&server).await;
        ...
        let client = create_mock_derivative_client(&server.uri());
        let result = client
            .translate("test-urn", OutputFormat::Svf2, None)
            .await;
    ...
    async fn test_translate_invalid_urn() {
        ...
        let client = create_mock_derivative_client(&server.uri());
        let result = client
            .translate("invalid-urn", OutputFormat::Svf2, None)
            .await;
    ...
    async fn test_get_manifest_success() {
        ...
        Mock::given(method("GET"))
            .and(path_regex(r"/modelderivative/v2/designdata/.+/manifest"))
            .and(header("Authorization", "Bearer test-token-12345"))
            .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({
                "type": "manifest",
                "hasThumbnail": "true",
        ...
        let client = create_mock_derivative_client(&server.uri());
        let result = client.get_manifest("test-urn").await;
    ...
    async fn test_get_manifest_pending() {
        ...
        assert!(result.is_ok());
        let manifest = result.unwrap();
        assert_eq!(manifest.status, "inprogress");
    ...
    async fn test_get_manifest_not_found() {
        ...
        Mock::given(method("GET"))
            ...
            .respond_with(ResponseTemplate::new(404).set_body_json(serde_json::json!({
                "diagnostic": "The specified resource does not exist."
            })))
            .mount(&server)
            .await;

        let client = create_mock_derivative_client(&server.uri());
        let result = client.get_manifest("nonexistent-urn").await;
    ...
    #[tokio::test]
    async fn test_delete_manifest_success() {
        let server = MockServer::start().await;
        setup_auth_mock(&server).await;
        ...
        let client = create_mock_derivative_client(&server.uri());
        let result = client.delete_manifest("test-urn").await;
    ...
    async fn test_delete_manifest_not_found() {
        ...
        Mock::given(method("DELETE"))
            .and(path_regex(r"/modelderivative/v2/designdata/.+/manifest"))
            .respond_with(ResponseTemplate::new(404).set_body_json(serde_json::json!({
                "diagnostic": "Manifest not found"
            })))
            .mount(&server)
            .await;

        let client = create_mock_derivative_client(&server.uri());
        let result = client.delete_manifest("nonexistent-urn").await;
    ...
    async fn test_get_status_success() {
        ...
        let client = create_mock_derivative_client(&server.uri());
        let result = client.get_status("test-urn").await;

        assert!(result.is_ok());
        let (status, progress) = result.unwrap();
        assert_eq!(status, "success");
        assert_eq!(progress, "complete");
    }

    #[tokio::test]
    async fn test_get_status_inprogress() {
        let server = MockServer::start().await;
        setup_auth_mock(&server).await;
    ...
    #[tokio::test]
    async fn test_list_downloadable_derivatives_success() {
        let server = MockServer::start().await;
        setup_auth_mock(&server).await;
                ...
                "derivatives": [
                    {
                        ...
                        "children": [
                            ...
                            },
                            {
                                ...
                                "role": "3d",
                                "name": "model.mtl",
                                "urn": "urn:derivative:obj:material",
                                "mime": "text/plain",
                                "size": 1024
                            }
                        ]
        ...
        let client = create_mock_derivative_client(&server.uri());
        let result = client.list_downloadable_derivatives("test-urn").await;

        assert!(result.is_ok());
        let derivatives = result.unwrap();
        assert_eq!(derivatives.len(), 2);
        assert_eq!(derivatives[0].name, "model.obj");
        assert_eq!(derivatives[0].size, Some(102400));
        assert_eq!(derivatives[1].name, "model.mtl");
    }

    #[tokio::test]
    async fn test_list_downloadable_derivatives_nested() {
        let server = MockServer::start().await;
        setup_auth_mock(&server).await;
                                ...
                                "children": [
                                    {
                                        ...
                                        "role": "graphics",
                                        "name": "output.svf",
                                        "urn": "urn:derivative:svf2:output",
                                        "mime": "application/autodesk-svf2"
                                    }
                                ]
                            }
        ...
        assert!(result.is_ok());
        let derivatives = result.unwrap();
        // Should find the nested child with urn
        assert_eq!(derivatives.len(), 1);
        assert_eq!(derivatives[0].guid, "child-guid");
    }

    #[tokio::test]
    async fn test_list_downloadable_derivatives_empty() {
        let server = MockServer::start().await;
        setup_auth_mock(&server).await;
    ...
    #[tokio::test]
    async fn test_translate_rate_limit() {
        let server = MockServer::start().await;
        setup_auth_mock(&server).await;
        ...
        Mock::given(method("POST"))
            ...
            .respond_with(ResponseTemplate::new(429).set_body_json(serde_json::json!({
                "diagnostic": "Rate limit exceeded. Please try again later."
            })))
            .mount(&server)
            .await;
    ...
    #[tokio::test]
    async fn test_translate_forbidden() {
        let server = MockServer::start().await;
        setup_auth_mock(&server).await;
        ...
        Mock::given(method("POST"))
            ...
            .respond_with(ResponseTemplate::new(403).set_body_json(serde_json::json!({
                "diagnostic": "Access denied to resource"
            })))
            .mount(&server)
            .await;
